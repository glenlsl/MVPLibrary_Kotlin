...
public class NiceSpinner extends AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        popupTextSize = typedArray.getDimension(R.styleable.NiceSpinner_popupTextSize, 15);
        popupWidth = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_popupWidth, -1);
        ...
        if (!isArrowHide) {
            Drawable basicDrawable = typedArray.getDrawable(R.styleable.NiceSpinner_arrowDrawable);
            if (basicDrawable == null) {
                basicDrawable = ContextCompat.getDrawable(context, R.drawable.arrow);
            }
    ...
    public int textToIndex(String text) {
        List<String> list = ((NiceSpinnerAdapter) adapter).getmItems();
    ...
    public <T> void attachDataSource(@NonNull List<T> dataset) {
        if (adapter == null) {
            adapter = new NiceSpinnerAdapter<>(getContext(), dataset, textColor, popupTextSize, backgroundSelector);
            setAdapterInternal(adapter);
        } else {
            try {
                ((NiceSpinnerAdapter) adapter).setDatas(dataset);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void setAdapter(@NonNull NiceSpinnerBaseAdapter adapter) {
        this.adapter = new NiceSpinnerAdapterWrapper(getContext(), adapter, textColor, popupTextSize, backgroundSelector);
